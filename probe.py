...
def trigger_uploader(config):
   ...
   try:
      # 获取当前 Python 解释器的路径
       ...
       uploader_path = config.get('UPLOADER', 'uploader_path')
...
def main():
   ...
   scheduler.add_job(probe_task, 'interval', seconds=config.getint('WEBSITE', 'probe_interval'), args=[config], next_run_time=datetime.now())
